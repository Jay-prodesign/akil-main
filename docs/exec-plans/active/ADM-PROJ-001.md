...
- **Correction 3** (commit `15815e96397a5a8148c78476769afa80a73ba05b`): fixed both Rev108 findings. Brain Rev109 confirmed the F1/F2 fixes themselves were materially correct, but found that the F2 fix introduced its own new defect: the compiler-boundary re-validation calls (`createAcceptedCommercialReference`/`createMaterialPlatformDecision`) were placed *before* Step 1's structural coherence checks, so a combined adversarial input carrying both a structural mismatch (e.g. wrong tenant) and an invalid hand-built commercial/platform record would throw `InvalidAcceptedCommercialReferenceError`/`InvalidMaterialPlatformDecisionError` instead of the required `InvalidProjectActivationProfileError` — violating Rev101's own explicit blocker-priority order ("structural mismatch throws first").
- **This fourth correction** (commit pending push) fixes the Rev109 finding: the boundary re-validation calls are moved to run immediately after Step 1's structural checks (and still before Step 2's commercial equality check), so structural coherence always wins the priority race. No Rev108 behavior was weakened — the same two factory calls still run unconditionally on every invocation, only their position moved. Two new combined-adversarial priority tests prove `InvalidProjectActivationProfileError` wins when a structural mismatch and an invalid hand-built record coexist; the existing coherent-input direct-object rejection tests (Rev108 F2) are retained unchanged.
- Per Rev106/107/108/109 ("Founder gate: NONE... Do not ask Founder for re-authorization... Do not idle for Founder dvm/continue"), all four correction rounds proceeded without a further Founder confirmation round-trip — Brain's own review is the authorizing verdict for this bounded correction lane.
...
- `compileProjectActivationProfile(input)` — the exact Rev101 nine-step order, full detail in the function's own doc comment:
  1. Structural coherence (`customer`/`project`/`ownership`/`soldScope` vs `tenantScope`) — throws. This remains the first failure class (Rev109 F1): the Rev108 F2 compiler-boundary re-validation (below) always runs *after* this step, so a combined structural-mismatch-plus-invalid-record input still fails on the structural error first.
  1.5. Compiler-boundary re-validation (Rev108 F2, reordered by Rev109 F1): re-invokes `createAcceptedCommercialReference(input.acceptedCommercialReference)` and, if supplied, `createMaterialPlatformDecision(input.platformDecision)`, discarding the return value — this makes both types' own construction invariants non-bypassable at the compiler boundary, not just at construction time via the factory a caller might skip.
...
- That boundary re-validation never outranks structural coherence: when a structural mismatch and an invalid hand-built commercial/platform record coexist in the same call, `InvalidProjectActivationProfileError` always wins (Rev109 F1 regression tests) — matching Rev101's explicit blocker-priority order.
...
- **A21 guard-disable sanity** (performed manually per Rev103's own precedent, not as a permanent test — first correction round only): the step-2 `sourceBlueprintVersion` equality guard was temporarily replaced with `false ||`, the suite was rebuilt and rerun — test A4 ("a commercial reference for the wrong blueprintVersion throws") failed exactly as expected, confirming the guard is load-bearing; the guard was then restored byte-for-byte (`diff` confirmed) and the full suite re-ran green. Every later round's new guards (Rev107's two; Rev108's three; Rev109's validation-order fix) are each self-proving by their own dedicated adversarial/regression tests, so per Rev107/108/109's identical conditional no further manual A21 exercise was required for any of them.
...
`tests/project-activation-profile.test.ts` (39 tests) — the full Rev101 "TEST MATRIX (minimum)" A1-A20 with Rev107/Rev108/Rev109's controlled/expanded coverage, plus construction-validation tests for `AcceptedCommercialReference`/`MaterialPlatformDecision`, F1/F2 regression tests (Rev108), two combined-adversarial priority tests (Rev109), and the boundary/dependency-delta checks folded in as `A20/boundary` (kept in this one file per Rev106's three-file bounded write surface, rather than a separate boundary-scan file):
...
| Rev109 F1 regression: a structural mismatch wins priority over a coexisting invalid hand-built commercial/platform record | 2 tests |
...
- Result: **747/747 tests pass** (708 pre-existing on `main@3226c76fa338e425e553638e5f5f48924182a1c0` + 39 in `project-activation-profile.test.ts`). Zero regressions, zero skips.
- First-round A21 guard-disable sanity performed and reverted as described above; full suite re-confirmed green after restore. Second, third, and fourth round (Rev107/Rev108/Rev109) guards are each self-proving by their own dedicated regression tests.

## Status

`IMPLEMENTED / SELF-VALIDATED`. Per `AGENTS.md` §10 and this repository's `CLAUDE.md`, Claude's authority ends here. This fourth correction addresses the Rev109 finding (F1) on top of the already-resolved Rev104, Rev107, and Rev108 findings; it stays **OPEN/DRAFT/HOLD_MERGE** on PR #101 pending independent Brain re-review of the new exact head. No merge, no MAIN mutation.
