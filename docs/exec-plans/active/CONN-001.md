...
## Self-found hardening correction: prototype-shaped connectionBindingId corrupts the durable dictionary

Self-initiated bounded hardening scan (per this corridor's own standing instruction to continue the admitted hardening/gap scan for concrete, evidenced defects whenever no Brain-issued task is pending) found and live-reproduced a genuine defect in `FileDurableConnectorConnectionStore`, the same bug *class* Brain has now found and fixed three separate times elsewhere in this repository (`worker-routing-policy.ts` Rev60 F1, `local-execution-hardening.ts`, `local-execution-collaboration.ts`) - a caller-controlled string reaching an unguarded plain-object property access, defeating an `=== undefined` / lookup check via the JavaScript prototype chain.

**Defect**: `connectionBindingId` is caller-supplied and validated only as a non-empty, whitespace-trimmed string (`createConnectionBinding`'s `requireNonEmptyString` in `connection-authority.ts`) - a legal value such as `"__proto__"` is never rejected. `readAll()`'s `result: StoredRecordsFile = {}` and the empty-file `return {}` were both plain-`Object.prototype`-based dictionaries. Live-reproduced: creating a connection with `connectionBindingId: "__proto__"` and calling `store.save(instance)` (no `expectedVersion`, a genuine first-time create) threw `ConnectorConnectionVersionConflictError` claiming a record already existed, because `records["__proto__"]` resolved through the inherited prototype chain to `Object.prototype` itself rather than `undefined`. Worse, a bracket *write* of that same key (`records[connectionBindingId] = created`) invokes `Object.prototype`'s special `__proto__` accessor setter, silently reassigning the in-memory dictionary's own prototype instead of adding a normal entry - corrupting the object and causing `JSON.stringify` to silently drop the write entirely on a subsequent persist.

**Fix**: `readAll()` now constructs both dictionaries via `Object.create(null)` instead of the `{}` literal. A null-prototype object inherits nothing from `Object.prototype` - no `__proto__` accessor, no `constructor`, no `toString` - so every key, including prototype-shaped ones, behaves as an ordinary own data property. `JSON.stringify`/`JSON.parse`/`Object.entries` all operate on own enumerable properties regardless of prototype, so this changes no observable serialization behavior for any ordinary key; `save`/`get`/`list` all operate on the object `readAll()` returns, so the fix closes the defect at its single source rather than needing repeated guards at each call site.

**Tests**: two new adversarial tests in `tests/durable-connector-connection-store.test.ts` - M16 (`connectionBindingId: "__proto__"` saves as a genuine version-1 create, round-trips correctly through `get`/`list`, and the persisted JSON file contains it as a real own key, not a corrupted/vanished prototype reassignment) and M17 (`connectionBindingId: "constructor"` coexists correctly alongside an ordinary key with no cross-contamination). **1724/1724 tests pass** (1722 base + 2 new), strict typecheck clean, clean `dist/` rebuild, zero new runtime dependency.

**Sanity-check disclosure**: reverted both `Object.create(null)` call sites to the original `{}` literal and reconfirmed exactly M16/M17 fail (16 pass/2 fail), nothing else; restored and reconfirmed the full suite green (1724/1724).

**Scope-and-topology**: branched from the then-current forward lineage tip (PR #94's exact head) for topology cleanliness, draft-targeting that branch. No `authority.ts`/tenant-isolation/payment/credential semantics touched - this is a pure dictionary-safety correction inside an already-tenant-scoped, already-lock-protected store; no other CONN-001 invariant (ownership-match, version-conflict, replay validation, secret-boundary) is altered.

**Brain Rev64 (PR #95 exact head `7ea1d374b6a88e5ec1530e17833a978e0c8f0d47`): PASS/VERIFIED.** Brain independently reviewed the bounded four-file patch and exact-head GitHub Actions run `35217984208` (SUCCESS): the defect is real and source-proven, the correction is minimal and accepted (`Object.create(null)` for both in-memory record dictionaries, preserving ordinary JSON/`Object.entries` semantics while eliminating inherited prototype keys/accessors), M16/M17 directly prove `__proto__` round-trip persistence and `constructor` coexistence, and no authority/tenant-boundary/payment/credential/provider semantic is widened. This closure is separate from and does not reopen the Rev63 bounded launch-software feature STOP_PROOF. `HOLD_MERGE` inherited; Founder gate NONE.

## Status

**IMPLEMENTED / SELF-VALIDATED (Rev94-corrected, plus a self-found prototype-safe-dictionary hardening correction, Brain Rev64 PASS/VERIFIED on that correction)** (slices 1-5 of CONN-001, F1-F3 corrected). Brain independent exact-head re-review of the base Rev94-corrected slices 1-5 remains pending (deferred to the end of the current continuous-execution batch per Rev95); the Rev64 hardening correction itself is closed PASS/VERIFIED. `MERGE_DISPOSITION: HOLD_MERGE` (no `MAIN` mutation). Remaining, all genuinely gated or explicitly deferred: real credential entry/OAuth exchange/live network calls (protected gate); `META` (deliberately deferred, not fabricated - see Rev94 F3 above). The Rev91/92/93/94 SALE-TO-CLOSE E2E acceptance floor is tracked separately in `docs/exec-plans/active/SALE-TO-CLOSE.md` on its own branch/PR.
