...
| 12 | Website Build Cold-Start end-to-end convergence | IMPLEMENTED/EVIDENCED (Brain Rev114-121-corrected: PR #58's closure-approval-gate correction is now actually composed into this tree — not merely cited — and the routing↔execution glue requires an explicit, admission-time-only `ExecutionRoutingRequirement` for every `EXECUTING` transition. Rev117 closed self-servable `MANUAL_EXECUTION_ALLOWED` construction by an ordinary-WRITE caller; Rev118/119 closed retroactive relabeling of an already-admitted job via `ExecutionRoutingRequirementRegistry`'s immutable-once-set admission; Rev120 found even the FIRST admission was still a bare caller-selected string and closed it by binding `ROUTING_REQUIRED` to a real `WorkerRoutingDecision` and `MANUAL_EXECUTION_ALLOWED` to a real `AdmittedWorker` with `authorityLevel: "ELEVATED"`; Rev121 found neither Rev120 primitive was actually exact-job-bound and closed it by requiring `admitRoutingRequiredFromAssignment` to consume a real, job-validated `RoutedExecutionAssignment` and `admitManualExecutionAllowedByAdmittedWorker` to additionally require `trustStatus: "ADMITTED"` plus a persisted `evidenceRef` — reusing existing primitives instead of a generic authority check or a fresh job-to-service binding — see the "Brain Rev121" commit on this branch) |
...
See `docs/engineering/CURRENT_STATE.md`'s own "AWAITING BRAIN" section for the current, single source of truth on what is pending — this note exists here only to point there rather than duplicate a value that changes on every push. As of the Rev121 correction: pending Brain's incremental HIGH/PROTECTED re-review of `ExecutionRoutingRequirementRegistry`'s two exact-job-bound admission methods (`admitRoutingRequiredFromAssignment` requires a real, job-validated `RoutedExecutionAssignment`; `admitManualExecutionAllowedByAdmittedWorker` requires a real `AdmittedWorker` with `trustStatus: "ADMITTED"` and `authorityLevel: "ELEVATED"` plus a persisted `evidenceRef`). No Founder action required.
