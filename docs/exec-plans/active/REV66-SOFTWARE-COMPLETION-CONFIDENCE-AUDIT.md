...
### 6. Acceptance-contract/source-to-test trace — SATISFIED (spot-checked, not exhaustive); one BLOCKED finding
...
**One genuine cross-reference discrepancy found and resolved (BLOCKED, not a new defect for this branch to fix):** `durable-connector-connection-store.ts`'s own doc comments (and `CURRENT_STATE.md`'s Rev94 entry) state that `FileDurableOutcomeJobStore.putIfAbsent` "already" uses a `linkSync`-based atomic-creation lock, citing `AUD-DURABILITY-GAP`. Reading the actual current `src/domain/durable-outcome-job-store.ts` on this branch shows **no locking at all** - `putIfAbsent` still does a plain read-check-write (`readFileSync`/`writeFileSync`), the exact TOCTOU race already fixed in its two sibling stores (`durable-connector-connection-store.ts` Rev94 F1, `durable-external-sale-bootstrap-store.ts`). Confirmed this is **not** a regression or an oversight specific to this branch: `git diff origin/main -- src/domain/durable-outcome-job-store.ts` shows `main` itself (`3226c76`) has the identical unfixed pattern. The actual fix genuinely exists - on **PR #30 (`AUD-DURABILITY-GAP`)**, branch `claude/aud-durability-gap-correction`, exact head `cefff35bbc444cdb77eff998565779e93ceafed0`, cut from the same `main` base, `state: open`, `draft: true`, `merged: false`, `IMPLEMENTED / SELF-VALIDATED` / `HOLD_MERGE` (independently confirmed live via the GitHub API before writing this). That PR closes the identical race via an OS-atomic `linkSync`-based per-`(tenantId, jobId)` lock, plus fail-closed replay revalidation for all three of `FileDurableEngineeringStore`/`FileDurablePlanAdmissionStore`/`FileDurableOutcomeJobStore` - it was never merged into `main`, so every branch cut from `main` since (including this one) still carries the original race. **Disposition: BLOCKED** - the correct action is merging PR #30, not re-implementing its already-correct, already-reviewed fix a second time on a third branch (which would just create a duplicate/conflicting implementation of the identical fix). This is surfaced here as a genuine completion-confidence finding: at least one Brain-accepted durability fix exists un-merged, and this doc-comment cross-reference is the kind of drift a completion-confidence audit is meant to catch. No source change made on this branch for this finding - merge disposition is a Founder/Brain-gated decision (`AGENTS.md`/`CLAUDE.md` merge-topology constraint), not an engineering implementation gap this branch can close.
