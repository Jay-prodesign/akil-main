...
- A secret-free TypeScript domain/application kernel under `src/domain/`, `src/application/`, `src/ports/`, `src/web/`, `src/fixtures/`, covering identity/session/tenant boundaries, delivery/outcome-job lifecycle, evidence/verification, organization/ownership/partner authority, commercial/attention/operations read-models, connector/integration control-plane primitives, worker/routing policy, cross-domain intelligence, internal command projection, and the founder-approved Local Execution Bridge (LOCAL-EXEC-001…007). See `docs/exec-plans/active/` and `docs/exec-plans/completed/` for the authoritative per-module record.
