...
| Workstream H: partner capability claim admission (pure lifecycle function; extends `V3-PTR-001`) | `V5-PTN-001` | `IMPLEMENTED / SELF-VALIDATED`, PR #21 open/`HOLD_MERGE`, pending Brain exact-head review. Closes §12's "capability claim cannot self-certify" gap: a `PartnerCapabilityClaim` is always born `UNVERIFIED`; only an explicit `admitPartnerCapabilityClaim` call (requiring non-empty evidence and a forward-looking review date) can produce `ADMITTED`; `EXPIRED` is a computed, never-stored disposition; `REVOKED` is terminal. Exactly one type-only import (`PartnerOrganization`), no system-clock read. 651/651 tests pass, strict typecheck clean, zero new dependency. |
