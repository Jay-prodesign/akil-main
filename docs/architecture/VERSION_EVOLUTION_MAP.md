...
| Workstream A: RESOLVE authority separation, extending the same `engineering-run-state.ts` reducer | `V5-ENG-002` | `IMPLEMENTED / SELF-VALIDATED`, pending Brain exact-head review. Enforces "agent cannot self-mark final VERIFIED" — a `RESOLVE` event is rejected unless `fromRole` is `BRAIN` or `OWNER`; empirically confirmed as a real, exploitable gap before the fix (a `WORKER`-authored `RESOLVE(PASS)` was previously accepted). No new event type, no second orchestration engine. |
