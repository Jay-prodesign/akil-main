...
| Workstream F: cross-domain company intelligence reconciliation floor (pure aggregation function; no persistent "strategic company memory" store) | `V5-INT-001` | `IMPLEMENTED / SELF-VALIDATED, Brain Rev50 PASS, merged` (PR #18, `839aca0f6dd830c46dcf4576e46035c3fdbd9207`). Combines opaque cross-domain insight pointers (closed six-surface enum matching §10's own text) into a governed per-subject snapshot — fails closed on tenant/project scope mismatch, empty/whitespace `sourceRef`, or a future-relative-to-`asOf` `capturedAt` (the latter two added in the Rev49 correction after Brain's exact-head review of the original submission found both gaps empirically exploitable), resolves `STALE`/`CONFLICTING`/`CURRENT` by caller-supplied freshness, never blurs `OBSERVED` into `DERIVED`. No real cross-domain data source invented; reuses the existing `TenantScope` isolation primitive only. 629/629 tests pass, strict typecheck clean, zero new dependency. |
| Workstream B: worker/model routing policy (pure executor/reviewer selection function; no real provider/model API) | `V5-WRK-001` | `IMPLEMENTED / SELF-VALIDATED, Brain Rev53 PASS, merged` (PR #20, `f03c917e66283fcd63736234d81b166465496d7c`). Selects an authorized executor (and, where required, an independent reviewer) from a caller-declared registry of admitted workers — fails closed on an untrusted/unavailable/incapable/under-authorized worker (including explicit tool-access, policy-constraint, and authority-level dimensions, added in the Rev52 correction after Brain found them missing), never lets cost or candidate order override policy, never lets the same worker review its own work. No real provider/model API, worker registry, or hard-coded vendor/model name anywhere in the module; zero imports, no system-clock read. 666/666 tests pass, strict typecheck clean, zero new dependency. |
| Workstream H: partner capability claim admission (pure lifecycle function; extends `V3-PTR-001`) | `V5-PTN-001` | `IMPLEMENTED / SELF-VALIDATED (Rev55 F1-corrected)`, exact head `e7013fac72989380568726f750518b9ddee39046` on base `f03c917e66283fcd63736234d81b166465496d7c`, pending fresh Brain exact-head review. Closes §12's "capability claim cannot self-certify" gap: a `PartnerCapabilityClaim` is always born `UNVERIFIED`; only an explicit `admitPartnerCapabilityClaim` call (requiring non-empty evidence, a forward-looking review date, and — per Rev55 F1 — a non-empty `admittingAuthorityId` fail-closed rejected when it equals the claim's own `partnerOrganizationId`) can produce `ADMITTED`; the accepted authority is persisted as `admittedByAuthorityId` for audit; `EXPIRED` is a computed, never-stored disposition relative to a caller-supplied `asOf`; `REVOKED` is terminal. Exactly one type-only import (`PartnerOrganization`), no system-clock read, no capability-catalog invented. 691/691 tests pass, strict typecheck clean, zero new dependency. |
| Workstream E, G, I-J | — not started | No infrastructure/provider orchestration, no reusable-capability learning history, no governed evaluation-loop history, no unified command frontend substrate — `AI Commerce` remains an explicitly separate, isolated project per `AGENTS.md`/`CLAUDE.md`. |
