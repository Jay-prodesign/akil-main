...
- **SLA/escalation/attention model** (V3-SLA-001): `attention-state.ts` — `InternalAttentionLevel`/`ContractualSlaStatus` (always `UNKNOWN`) + `AttentionState`/`buildAttentionState`, reusing `resolveCurrentOwner` (V3-OWN-001). Imports `outcome-job.ts` as types only (no transition function) and nothing from `authority.ts`.
