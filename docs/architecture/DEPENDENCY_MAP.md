...
- `commercial-authority.ts` (V3-COM-001, PR `#5`, not yet merged) depends on `tenant-scope.ts`, `project-ownership.ts` (V2-CDO-003), and `ownership-assignment.ts` (V3-OWN-001, `resolveCurrentOwner` reused directly for `DEAL_OWNER` resolution). It deliberately does NOT import `outcome-job.ts`, `authority.ts`, or `partner-organization.ts`.
- `service-capability-routing.ts` (V4-SVC-001, PR `#6`, not yet merged) depends on `tenant-scope.ts`, `project-ownership.ts` (V2-CDO-003), and `capability-admission.ts` (V2-CDO-004, `CapabilityAdmission` type reused directly — no create/promote function imported).
