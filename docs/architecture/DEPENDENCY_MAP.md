...
- `operations-attention.ts` (V4-OPS-001) depends only on `tenant-scope.ts` and `attention-state.ts` (V3-SLA-001, `AttentionState`/`InternalAttentionLevel` types reused directly). It deliberately does NOT import `outcome-job.ts` (only `attention-state.ts`'s own already-composed types are used, keeping this module one layer removed from job-transition capability), nor `authority.ts`, `capability-admission.ts`, `connection-authority.ts`, `service-capability-routing.ts`, or `partner-organization.ts`.
