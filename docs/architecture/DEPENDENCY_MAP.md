...
- `commercial-authority.ts` (V3-COM-001, PR #5, open) depends only on `tenant-scope.ts`, `project-ownership.ts` (V2-CDO-003), and `ownership-assignment.ts` (V3-OWN-001, `resolveCurrentOwner`/`OwnershipAssignment` reused directly for an optional `dealOwnerMembershipId`). It imports nothing from `outcome-job.ts`, `authority.ts`, or `partner-organization.ts`.
