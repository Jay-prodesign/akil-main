...
import * as PartnerCapabilityAdmission from "../src/domain/partner-capability-admission.js";

// This file lives at <repo-root>/tests/v5-ptn-001-boundary-scan.test.ts.
const REPO_ROOT = join(dirname(fileURLToPath(import.meta.url)), "..", "..");
const V5_PTN_001_FILE = "src/domain/partner-capability-admission.ts";
...
test("V5-PTN-001: partner-capability-admission.ts contains no secret material or hard-coded provider/model coupling", () => {
...
test("V5-PTN-001: createPartnerCapabilityClaim is the only export capable of constructing a PartnerCapabilityClaim, and it accepts no status/evidence field", () => {
  const content = readFileSync(join(REPO_ROOT, V5_PTN_001_FILE), "utf8");
  const constructorMatches = content.match(/^export function \w+\(/gm) ?? [];
  assert.deepEqual(constructorMatches.sort(), [
    "export function admitPartnerCapabilityClaim(",
    "export function createPartnerCapabilityClaim(",
    "export function resolvePartnerCapabilityClaimStatus(",
    "export function revokePartnerCapabilityClaim(",
  ]);
});

test("V5-PTN-001: module exports exactly the expected surface - no execution, mutation-of-record-elsewhere, or clock-reading function", () => {
  const exportedKeys = Object.keys(PartnerCapabilityAdmission).sort();
...
test("V5-PTN-001: package.json still declares no new runtime dependency", () => {
