...
// This file lives at <repo-root>/tests/durable-plan-admission-store-concurrency.test.ts
// and is compiled to dist/tests/..., alongside the also-compiled
// dist/tests/helpers/plan-admission-store-append-worker.js.
const HERE = dirname(fileURLToPath(import.meta.url));
const WORKER_SCRIPT = join(HERE, "helpers", "plan-admission-store-append-worker.js");
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (code === 0) {
        ...
        reject(new Error(`plan-admission-store-append-worker exited with code ${code}`));
      }
    });
  });
}
...
test(
  "Rev71 (repo-wide Rev66 audit closure): real OS-level concurrent processes appending distinct ANSWER_RECORDED events to the same plan's durable log never lose an update - the prior read-modify-write implementation could silently drop a concurrent writer's event",
  { timeout: 30_000 },
  async () => {
    const storeDir = freshDir("plan-admission-store-race-");
    const readyDir = freshDir("plan-admission-store-race-ready-");
    const goFile = join(freshDir("plan-admission-store-race-go-"), "go");
    try {
      ...
      const events = store.getEvents(
        "tenant-plan-admission-race" as never,
        "customer-plan-admission-race" as never,
        "project-plan-admission-race" as never,
        "plan-plan-admission-race" as never,
      );
      assert.equal(
        events.length,
        WORKER_COUNT,
