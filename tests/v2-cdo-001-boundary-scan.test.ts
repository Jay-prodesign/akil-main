...
// This file lives at <repo-root>/tests/v2-cdo-001-boundary-scan.test.ts.
const REPO_ROOT = join(dirname(fileURLToPath(import.meta.url)), "..", "..");

const V2_CDO_001_FILES = [
  "src/domain/delivery-status.ts",
  "src/domain/delivery-timeline.ts",
];
...
test("V2-CDO-001: delivery-status/delivery-timeline source contains no secret material or commerce-provider coupling", () => {
  const violations: string[] = [];
  for (const relativePath of V2_CDO_001_FILES) {
...
test("V2-CDO-001: delivery-status/delivery-timeline source has no model/provider hard-coding or queue/workflow-engine/AI-Commerce coupling", () => {
  const violations: string[] = [];
  for (const relativePath of V2_CDO_001_FILES) {
    const content = readFileSync(join(REPO_ROOT, relativePath), "utf8");
...
test("V2-CDO-001: package.json still declares no new runtime dependency", () => {
