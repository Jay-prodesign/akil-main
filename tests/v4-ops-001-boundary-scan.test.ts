...
test("V4-OPS-001: operations-attention.ts imports outcome-job.ts only through attention-state.ts's own re-exported types, and nothing from src/web/ or src/application/", () => {
  const content = readFileSync(join(REPO_ROOT, "src/domain/operations-attention.ts"), "utf8");
  const importLines = content.split("\n").filter((line) => /^\s*import\b/.test(line));
  const violatingOutcomeJobImport = importLines.find((line) => /["']\.\/outcome-job\.js["']/.test(line));
  const violatingWebAppImport = importLines.find((line) => /\.\.\/(web|application)\//.test(line));
  assert.equal(violatingOutcomeJobImport, undefined);
