...
// This file lives at <repo-root>/tests/v3-sla-001-boundary-scan.test.ts.
const REPO_ROOT = join(dirname(fileURLToPath(import.meta.url)), "..", "..");

const V3_SLA_001_FILES = ["src/domain/attention-state.ts"];
...
const HARD_NON_SCOPE_PATTERNS: ReadonlyArray<{ label: string; pattern: RegExp }> = [
  ...
  { label: "uptime-promise", pattern: /\b99\.\d+%|uptime guarantee/i },
  { label: "24/7-promise", pattern: /24\/7/ },
  { label: "penalty-credit", pattern: /penalty|service credit/i },
];

test("V3-SLA-001: attention-state.ts contains no secret material or commerce-provider coupling", () => {
  const violations: string[] = [];
  for (const relativePath of V3_SLA_001_FILES) {
...
test("V3-SLA-001: attention-state.ts has no model/provider hard-coding and no inferred uptime/24-7/penalty/commission/payout promise (no invented service commitment)", () => {
  const violations: string[] = [];
  for (const relativePath of V3_SLA_001_FILES) {
    const content = readFileSync(join(REPO_ROOT, relativePath), "utf8");
...
test("V3-SLA-001: attention-state.ts imports outcome-job.ts as types only - no create/transition function, so it cannot itself transition an OutcomeJob", () => {
  const content = readFileSync(join(REPO_ROOT, "src/domain/attention-state.ts"), "utf8");
  const importLines = content.split("\n").filter((line) => /^\s*import\b/.test(line));
  const outcomeJobImport = importLines.find((line) => /["']\.\/outcome-job\.js["']/.test(line));
  assert.notEqual(outcomeJobImport, undefined);
  assert.match(outcomeJobImport as string, /^\s*import\s+type\b/);
});

test("V3-SLA-001: attention-state.ts has no import of authority.ts", () => {
  const content = readFileSync(join(REPO_ROOT, "src/domain/attention-state.ts"), "utf8");
  const importLines = content.split("\n").filter((line) => /^\s*import\b/.test(line));
...
test("V3-SLA-001: attention-state.ts imports nothing from src/web/ or src/application/ (domain depends on nothing else in src/)", () => {
  const content = readFileSync(join(REPO_ROOT, "src/domain/attention-state.ts"), "utf8");
  const importLines = content.split("\n").filter((line) => /^\s*import\b/.test(line));
...
test("V3-SLA-001: package.json still declares no new runtime dependency", () => {
