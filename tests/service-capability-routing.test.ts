...
test("VERIFIED_AVAILABLE admission + a matching VERIFIED connectionBinding resolves READ_ONLY execution maturity, never a higher rung", () => {
  ...
  assert.match(route.reason, /VERIFIED/);
});

test("Rev28 bounded correction: VERIFIED_AVAILABLE admission with NO connectionBinding supplied resolves UNAVAILABLE - a stale/unproven admission is never promoted to READ_ONLY", () => {
  const route = resolveServiceCapabilityRoute({
    ownership: WEBSITE_BUILD_V1_OWNERSHIP,
    serviceFamilyRef: "website-build-v1",
    admission: WEBSITE_BUILD_V1_CAPABILITY_ADMISSION,
  ...
  assert.match(route.reason, /no ConnectionBinding was supplied/);
});

test("Rev28 bounded correction: a connectionBinding whose connectionBindingId does not match the admission's own connectionBindingId fails closed (capability/provider reference mismatch)", () => {
  const otherRequirement = createConnectionRequirement({
    connectionRequirementId: "conn-req-unrelated-other",
    ownership: WEBSITE_BUILD_V1_OWNERSHIP,
    requiredCapabilityRef: "required-access-connections",
    purpose: "an unrelated connection requirement for adversarial mismatch coverage",
    accountOwner: "CUSTOMER_OWNED",
    minimumProviderScope: [],
    connectionMethod: "OAUTH",
    validationRequirement: "provider-reported connection health check",
  });
  const otherRequested = createConnectionBinding({
    connectionBindingId: "conn-binding-unrelated-other",
    requirement: otherRequirement,
    ownership: WEBSITE_BUILD_V1_OWNERSHIP,
    providerRef: "reference-other-provider",
    workspaceRef: "reference-other-workspace",
    integrationInstanceRef: "reference-other-instance-1",
    delegatedScope: [],
  });
  const otherConnectedUnverified = transitionConnectionBinding(otherRequested, "CONNECTED_UNVERIFIED");
  const mismatchedBinding: ConnectionBinding = verifyConnectionBinding(
    otherConnectedUnverified,
    "internal://reference-fixtures/unrelated-other/connection-health-check",
  );
  ...
  const route = resolveServiceCapabilityRoute({
    ...
    serviceFamilyRef: "website-build-v1",
    admission: WEBSITE_BUILD_V1_CAPABILITY_ADMISSION,
    connectionBinding: mismatchedBinding,
  });
  assert.equal(route.executionMaturity, "UNAVAILABLE");
  assert.match(route.reason, /does not match the admission's own connectionBindingId/);
});

test("Rev28 bounded correction: a matching connectionBinding that is DEGRADED (not VERIFIED) fails closed - degraded provider connectivity is never promoted to READ_ONLY", () => {
  const degradedBinding: ConnectionBinding = {
    ...WEBSITE_BUILD_V1_CONNECTION_BINDING,
  ...
  const route = resolveServiceCapabilityRoute({
    ...
    serviceFamilyRef: "website-build-v1",
    admission: WEBSITE_BUILD_V1_CAPABILITY_ADMISSION,
    connectionBinding: degradedBinding,
  });
  assert.equal(route.executionMaturity, "UNAVAILABLE");
  assert.match(route.reason, /DEGRADED, not VERIFIED/);
});

test("Rev28 bounded correction: a matching connectionBinding that is REVOKED (stale connection) fails closed", () => {
  const revokedBinding: ConnectionBinding = {
    ...WEBSITE_BUILD_V1_CONNECTION_BINDING,
  ...
  const route = resolveServiceCapabilityRoute({
    ...
    serviceFamilyRef: "website-build-v1",
    admission: WEBSITE_BUILD_V1_CAPABILITY_ADMISSION,
    connectionBinding: revokedBinding,
  });
  assert.equal(route.executionMaturity, "UNAVAILABLE");
  assert.match(route.reason, /REVOKED, not VERIFIED/);
});

test("Rev28 bounded correction: a connectionBinding belonging to a different project (adversarial cross-project substitution) fails closed even when its connectionBindingId happens to match", () => {
  ...
  const foreignBinding: ConnectionBinding = {
    ...WEBSITE_BUILD_V1_CONNECTION_BINDING,
  ...
  const route = resolveServiceCapabilityRoute({
    ...
    serviceFamilyRef: "website-build-v1",
    admission: WEBSITE_BUILD_V1_CAPABILITY_ADMISSION,
    connectionBinding: foreignBinding,
  });
  assert.equal(route.executionMaturity, "UNAVAILABLE");
  assert.match(route.reason, /does not belong to the given ownership scope/);
});
