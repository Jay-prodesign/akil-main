...
/**
 * AUD-DURABILITY-GAP (Rev68 forward-port): re-validates an already-persisted
 * `OutcomeJob` (e.g. read back from `FileDurableOutcomeJobStore`, including
 * from a creation-lock file) against this type's own shape, rather than
 * trusting a blind `JSON.parse(...) as OutcomeJob` cast on replay.
 * Deliberately not a reuse of `createOutcomeJob`: that constructor only
 * ever produces a fresh DRAFT job from live tenant/customer/project inputs
 * and cannot reconstruct a job already advanced to any other lifecycle
 * state, so this validator independently checks all seven persisted
 * fields, including that `state` is one of the eleven known
